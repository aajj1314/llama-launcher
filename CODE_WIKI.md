...
Llama Launcher 是一个基于 Python 的 cyberpunk 风格 TUI 启动器，用于管理和启动 llama.cpp 模型。它提供实时性能指标监控、多种运行模式和跨平台支持，为用户提供直观的模型管理体验。
...
1. **状态管理模块** (`state_manager.py`) - 作为核心，管理应用状态，为 TUI 和 WebUI 提供共享状态
2. **TUI 模块** (`run.py`) - 提供终端用户界面，用于模型选择和配置
3. **WebUI 模块** (`web_app.py`) - 提供基于 FastAPI 的 Web 界面和 RESTful API
4. **启动模块** (`launcher.py`) - 统一启动脚本，支持单独或同时启动 TUI 和 WebUI
...
#### `run_cli()`, `run_server()`, `run_embedding()` 函数
...
| llama.cpp | 安装于 `/home/anan/llama.cpp` | 底层模型运行库 | [state_manager.py](file:///workspace/state_manager.py) |
...
| 模块 | 用途 | 来源 |
|------|------|------|
| os | 文件系统操作 | [run.py](file:///workspace/run.py), [state_manager.py](file:///workspace/state_manager.py), [web_app.py](file:///workspace/web_app.py) |
| sys | 系统操作 | [run.py](file:///workspace/run.py), [launcher.py](file:///workspace/launcher.py) |
| subprocess | 启动子进程 | [run.py](file:///workspace/run.py), [web_app.py](file:///workspace/web_app.py) |
...
| re | 正则表达式 | [run.py](file:///workspace/run.py), [web_app.py](file:///workspace/web_app.py) |
...
1. **添加新的运行模式**：
   ...
   - 在 `run.py` 和 `web_app.py` 中添加相应的启动函数

2. **添加新的配置选项**：
   - 在 `state_manager.py` 中添加新的选项常量
